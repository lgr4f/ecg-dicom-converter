...
from scripts.dicom_validator.spec_reader.edition_reader import EditionReader
from scripts.dicom_validator.validator.dicom_file_validator import DicomFileValidator
...
def validate(args, base_path):
    ...
    for dicom_path in args.dicomfiles:
        results_error = validator.validate(dicom_path)
        error_nr += sum(
            len(error) for error in list(results_error.values())
        )
